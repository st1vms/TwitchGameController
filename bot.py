...
class GameBotController:
    ...
    async def __on_message(self, msg: ChatMessage):
        ...
        if text in self.controller.controls:
            await self.COMMAND_QUEUE.put(self.controller.controls[text])
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            while True:
                try:
                    task = await asyncio.wait_for(self.COMMAND_QUEUE.get(), timeout=0.5)
                    await task()
                except asyncio.TimeoutError:
                    c += 0.5
                    if c >= self.HELP_PING_TIMEOUT_SEC:
                        c = 0
